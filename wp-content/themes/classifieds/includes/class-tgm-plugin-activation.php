    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'oops'                           => esc_html__( 'Something went wrong.', 'tgmpa' ),
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <form id="tgmpa-plugins" action="" method="post">
                    <input type="hidden" name="tgmpa-page" value="<?php echo esc_attr__( $this->menu ); ?>" />
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr__( $this->strings['return'] ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . esc_html__( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    foreach ( $plugin_groups as $plugin_group_single_name ) {
                        ...
                        if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                            $linked_plugin_groups[] = '<a href="' . esc_url( $external_url ) . '" title="' . esc_attr__( $plugin_group_single_name ) . '" target="_blank">' . $plugin_group_single_name . '</a>';
                        }
                        elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                            ...
                            $linked_plugin_groups[] = '<a href="' . esc_url( $url ) . '" class="thickbox" title="' . esc_attr__( $plugin_group_single_name ) . '">' . $plugin_group_single_name . '</a>';
                        }
                        else {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( $external_url && preg_match( '|^http(s)?://|', $external_url ) ) {
                    $table_data[$i]['plugin'] = '<strong><a href="' . esc_url( $external_url ) . '" title="' . esc_attr__( $plugin['name'] ) . '" target="_blank">' . $plugin['name'] . '</a></strong>';
                }
                elseif ( ! $source || preg_match( '|^http://wordpress.org/extend/plugins/|', $source ) ) {
                    $url = add_query_arg(
                        array(
                    ...
                    $table_data[$i]['plugin'] = '<strong><a href="' . esc_url( $url ) . '" class="thickbox" title="' . esc_attr__( $plugin['name'] ) . '">' . $plugin['name'] . '</a></strong>';
                }
                else {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = esc_html__( 'Private Repository', 'tgmpa' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = esc_html__( 'Pre-Packaged', 'tgmpa' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = esc_html__( 'WordPress Repository', 'tgmpa' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? esc_html__( 'Required', 'tgmpa' ) : esc_html__( 'Recommended', 'tgmpa' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Not Installed', 'tgmpa' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Installed But Not Activated', 'tgmpa' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'tgmpa' ) . ' %2$s">' . esc_html__( 'Install', 'tgmpa' ) . '</a>',
                        esc_url(
                            wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'tgmpa' ) . ' %2$s">' . esc_html__( 'Activate', 'tgmpa' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
        ...
        public function column_cb( $item ) {
            ...
            if ( esc_html__( 'Private Repository', 'tgmpa' ) === $item['source'] ) {
                // Escape external URLs
                $plugin_url = esc_url( $plugin_url );
            } elseif ( esc_html__( 'Pre-Packaged', 'tgmpa' ) === $item['source'] ) {
            ...
            return sprintf( '<input type="checkbox" name="%1$s[]" value="%2$s" id="%3$s" />', esc_attr__( $this->_args['singular'] ), esc_attr__( $value ), esc_attr__( $item['sanitized_plugin'] ) );
        }
        ...
        public function no_items() {
            ...
            printf( esc_html__( 'No plugins to install or activate.', 'tgmpa' ).' <a href="%1$s" title="Return to the Dashboard">'.esc_html( 'Return to the Dashboard', 'tgmpa' ).'</a>', network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'status' => esc_html__( 'Status', 'tgmpa' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => esc_html__( 'Install', 'tgmpa' ),
                'tgmpa-bulk-activate' => esc_html__( 'Activate', 'tgmpa' ),
            );
            ...
            class TGM_Bulk_Installer extends WP_Upgrader {
                ...
                public function run( $options ) {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package']          = esc_html__( 'Install package not available.', 'tgmpa' );
                        $this->strings['downloading_package'] = esc_html__( 'Downloading install package from <span class="code">%s</span>&#8230;', 'tgmpa' );
                        $this->strings['unpack_package']      = esc_html__( 'Unpacking the package&#8230;', 'tgmpa' );
                        $this->strings['installing_package']  = esc_html__( 'Installing the plugin&#8230;', 'tgmpa' );
                        $this->strings['process_failed']      = esc_html__( 'Plugin install failed.', 'tgmpa' );
                        $this->strings['process_success']     = esc_html__( 'Plugin installed successfully.', 'tgmpa' );
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                            ...
                            $this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'tgmpa' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'tgmpa' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'tgmpa' ) . '</span>.</a>';
                    ...
                    public function before( $title = '' ) {
                        ...
                        echo '<div class="update-messages hide-if-js" id="progress-' . esc_attr__( $this->upgrader->update_current ) . '"><p>';
                    ...
                    public function bulk_footer() {
                        ...
                        foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                            if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( TGM_Plugin_Activation::$instance->menu ), network_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr__( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . TGM_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                                $complete[] = $plugin;
                                break;
                            }
                            // Nothing to store.
                        ...
                        if ( empty( $complete ) ) {
                            echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'tgmpa' ) . '">' . esc_html__( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
