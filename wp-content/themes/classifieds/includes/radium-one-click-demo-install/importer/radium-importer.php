 ...
 if ( !class_exists( 'Radium_Theme_Importer' ) ) {
	...
	class Radium_Theme_Importer {
        ...
        public function check_previous_meta( $continue, $post_id, $meta_key, $meta_value, $unique ) {
        	if( !empty( $_POST['_wp_http_referer'] ) && !stristr( $_POST['_wp_http_referer'], '?post=' ) ){
			...
			<div style="background-color: #F5FAFD; margin:10px 0;padding: 5px 10px;color: #0C518F;border: 2px solid #CAE0F3; clear:both; width:90%; line-height:18px;">
			 ...
			 if( !empty($this->imported_demos) ) { ?>
			  	...
			  	<div style="background-color: #FAFFFB; margin:10px 0;padding: 5px 10px;color: #8AB38A;border: 2px solid #a1d3a2; clear:both; width:90%; line-height:18px;">
			  		<p><?php esc_html_e('Demo already imported', 'radium'); ?></p>
			  	</div><?php
			   	//return;
	       ...
	       <div class="radium-importer-wrap" data-demo-id="1"  data-nonce="<?php echo wp_create_nonce('radium-demo-code'); ?>">
		        ...
		        <form method="post">
		        	<?php $this->intro_html(); ?>
		          	...
		          	<input name="reset" class="panel-save button-primary radium-import-start" type="submit" value="<?php echo esc_attr__( $button_text ) ; ?>" />
	    ...
	    /* CF FIELDS */
	    public function import_cf_fields() {

			if( !isset( $_POST ) ){
				$_POST = array();
			}

			$_POST['cf_values'] = file_get_contents( $this->demo_files_path . 'cf_fields.txt' );
			classifieds_import_cf_values();
	    }	    
	    ...
	    public function import_widgets( $data ) {
	    	...
	    	foreach ( $data as $sidebar_id => $widgets ) {
	    		...
	    		foreach ( $widgets as $widget_instance_id => $widget ) {
	    			...
	    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = !empty( $widget->title ) ? $widget->title : ''; // show "No Title" if widget instance is untitled
