...
function classifieds_update_ad(){
	...
	if( $ad_terms ){
		if( !empty( $ad_title ) && strlen($ad_title) > 9 ) {
			if( !empty( $ad_description ) && strlen(strip_tags($ad_description)) > 9 ){			
				...
				$response = '<div class="alert alert-danger">'.esc_html__( 'Description of the add should be minimum of 10 characters.', 'classifieds' ).'</div>';
			}
		} else {
			$response = '<div class="alert alert-danger">'.esc_html__( 'Ad title should be minimum of 10 characters.', 'classifieds' ).'</div>';
		}
