...
class SM_Datetime_Timestamp_Field extends SM_Field {
	...
	public function html() { ?>		
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'sm_text_small sm_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'sm_text_small sm_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
	<?php }
	public function parse_save_values() {
...
class SM_GMap extends SM_Field {
	public function enqueue_scripts() {
		parent::enqueue_scripts();
		wp_enqueue_script( 'sm-gmap', trailingslashit( SM_URL ) . 'js/field.gmap.js', array( 'jquery' ) );
		wp_enqueue_script( 'sm-google-map', 'https://maps.googleapis.com/maps/api/js?libraries=places&sensor=false' );
	}
	public function html() { 
		$name = str_replace( '[]', '', $this->name );
		$ad_gmap_longitude = '';
		$ad_gmap_latitude = '';
		if( !empty( $_GET['post'] ) ){
			$ad_gmap_longitude = get_post_meta( $_GET['post'], $name.'_longitude', true );
			$ad_gmap_latitude = get_post_meta( $_GET['post'], $name.'_latitude', true );
		}
		?>
		<div class="gmap">
			<input type="hidden" class="longitude" name="<?php echo $name ?>_longitude" value="<?php echo esc_attr( $ad_gmap_longitude ) ?>">
			<input type="hidden" class="latitude" name="<?php echo $name ?>_latitude" value="<?php echo esc_attr( $ad_gmap_latitude ) ?>">
			<input type="text" class="gmap_input">
			<div id="map"></div>
		</div>
	<?php }
	public function save( $post_id, $values ){
		$name = str_replace( '[]', '', $this->name );
		delete_post_meta( $post_id, $name.'_longitude' );
		delete_post_meta( $post_id, $name.'_latitude' );
		add_post_meta( $post_id, $name.'_longitude', $_POST[$name.'_longitude'] );
		add_post_meta( $post_id, $name.'_latitude', $_POST[$name.'_latitude'] );
	}
}
 ...
 *
 */
class SM_Checkboxes extends SM_Field {
	public function title() {}
	public function html() {
		...
		if ( $this->has_data_delegate() )
			$this->args['options'] = $this->get_delegate_data(); ?>
			<?php foreach ( $this->args['options'] as $key => $value ): ?>
			<input <?php $this->id_attr( 'item-' . $key ); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr(); ?> type="checkbox" <?php printf( 'name="%s"', esc_attr( $name ) ); ?>   value="<?php echo esc_attr( $key ); ?>" <?php echo in_array( $key, (array)$this->get_value() ) ? 'checked="checked"' : ''; ?> />
			<label <?php $this->for_attr( 'item-' . $key ); ?> style="margin-right: 20px;">
				<?php echo esc_html( $value ); ?>
			</label>
			<?php endforeach; ?>
 ...
 *
 */
class SM_Taxonomy_Checkboxes extends SM_Field {
	public function title() {}
	public function organize( Array &$cats, Array &$into, $parentId = 0 ){
	    ...
	    foreach ($into as $topCat) {
	        ...
	        $this->organize($cats, $topCat->children, $topCat->term_id);
	    }
	
	}
	public function show_children( $children, $name ){
		?>
		<div class="taxonomy-subgroup">
			<?php
			foreach( $children as $child ){
				?>
					<div class="tax-checkbox">
						<input <?php $this->id_attr( 'item-' . $child->slug ); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr(); ?> type="checkbox" <?php printf( 'name="%s"', esc_attr( $name ) ); ?>   value="<?php echo esc_attr( $child->slug ); ?>" <?php echo in_array( $child->slug, (array)$this->get_value() ) ? 'checked="checked"' : ''; ?> />					
						<label <?php $this->for_attr( 'item-' . $child->slug ); ?> style="margin-right: 20px;">
							<?php echo esc_html( $child->name ); ?>
						</label>
					</div>
					<?php
					if( !empty( $child->children ) ){
						$this->show_children( $child->children, $name );
					 }
					?>				
				<?php
			}
	...
	public function html() {
		$cats = get_terms( $this->args['taxonomy'], array('hide_empty' => false) );
		if( !empty( $cats ) ){
			$organized = array();
			$this->organize( $cats, $organized );
			$name = $this->get_the_name_attr();
			$name .= '[]';
			foreach( $organized as $cat ){
				?>
				<div class="taxonomy-group">
					<div class="tax-checkbox">
						<input <?php $this->id_attr( 'item-' . $cat->slug ); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr(); ?> type="checkbox" <?php printf( 'name="%s"', esc_attr( $name ) ); ?>   value="<?php echo esc_attr( $cat->slug ); ?>" <?php echo in_array( $cat->slug, (array)$this->get_value() ) ? 'checked="checked"' : ''; ?> />					
						<label <?php $this->for_attr( 'item-' . $cat->slug ); ?> style="margin-right: 20px;">
							<?php echo esc_html( $cat->name ); ?>
						</label>
					</div>
					<?php
					if( !empty( $cat->children ) ){
						$this->show_children( $cat->children, $name );
					 }
					?>
				</div>
				<?php
